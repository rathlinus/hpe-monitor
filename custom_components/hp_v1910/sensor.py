...
async def async_setup_entry(
    ...
    if "poe_ports" in coordinator.data:
        for port_data in coordinator.data["poe_ports"]:
            port_name = port_data.get("name", "Unknown")
            # Add current power sensor
            entities.append(
                HPV1910PortPoESensor(coordinator, config_entry, port_name)
            )
            # Add energy consumption sensor (kWh)
            entities.append(
                HPV1910PortPoEEnergySensor(coordinator, config_entry, port_name)
            )
...
class HPV1910PortPoEEnergySensor(CoordinatorEntity[HPV1910DataCoordinator], SensorEntity):
    """Representation of a HP V1910 port PoE energy consumption sensor (kWh)."""
    ...
    def __init__(
        ...
        self._port_name = port_name
        self._port_number = self._extract_port_number(port_name)
        self._attr_unique_id = f"{config_entry.entry_id}_{port_name}_poe_energy"
    ...
    @property
    def native_value(self) -> float | None:
        """Return the cumulative PoE energy consumption in kWh."""
        port_energy = self.coordinator.data.get("port_energy_kwh", {})
        energy = port_energy.get(self._port_name)
    ...
    def extra_state_attributes(self) -> dict[str, Any] | None:
        ...
        port_energy = self.coordinator.data.get("port_energy_kwh", {})
        energy_kwh = port_energy.get(self._port_name, 0.0)
        
        # Get current power for reference
        poe_ports = self.coordinator.data.get("poe_ports", [])
        current_power = 0.0
        for port in poe_ports:
            if port.get("name") == self._port_name:
                current_power = port.get("power_watts", 0.0)
                break
        
        return {
            "current_power_watts": current_power,
            "total_energy_wh": round(energy_kwh * 1000, 2),
            "port_name": self._port_name,
        }
